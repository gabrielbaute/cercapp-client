...
import PaymentHistoryChart from '../../components/common/PaymentHistoryChart.vue'; // Reutilizamos el componente maestro

export default defineComponent({
  name: 'DashboardView',
  components: { PaymentHistoryChart }, // Lo registramos
  setup() {
    return { ...useDashboard() };
  }
});
...
<template>
  <div class="space-y-6">
    ...
    <div v-else class="space-y-8">
      ...
      <div v-if="pagos.filter(p => p.status === 'COMPLETED').length > 0" class="bg-white p-6 rounded-2xl shadow-sm border border-cercapp-light h-96">
        <PaymentHistoryChart :payments="pagos" title="Evolución de tu Capital" />
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-cercapp-light flex items-center gap-4">
          <div class="w-12 h-12 bg-blue-50 text-cercapp-navy rounded-full flex items-center justify-center shrink-0">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-cercapp-light flex items-center gap-4">
          <div class="w-12 h-12 bg-yellow-50 text-cercapp-gold rounded-full flex items-center justify-center shrink-0">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-cercapp-light flex items-center gap-4">
          <div class="w-12 h-12 bg-gray-50 text-gray-600 rounded-full flex items-center justify-center shrink-0">
