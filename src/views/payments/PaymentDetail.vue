    ...
    <div v-else-if="payment" class="bg-white rounded-2xl shadow-sm border border-cercapp-light overflow-hidden">
      ...
      <div v-if="payment.status === 'PENDING'" class="p-6 bg-gray-50 border-t border-gray-100 flex justify-end">
        <button 
          ...
          class="bg-white border border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 font-semibold py-2 px-6 rounded-lg transition-colors flex items-center justify-center gap-2"
        >
          <span v-if="isCancelling" class="animate-spin h-4 w-4 border-2 border-red-600 border-t-transparent rounded-full"></span>
