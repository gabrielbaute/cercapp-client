...
<template>
  <div class="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
    <div class="flex items-center gap-4">
      ...
      <div>
        <p class="font-bold text-cercapp-navy capitalize">{{ formattedMethod }}</p>
        <p class="text-xs text-gray-500">{{ payment.payment_date }} • Ref: {{ payment.transaction_id || 'N/A' }}</p>
      </div>
    </div>

    <div class="flex items-center gap-4 text-right">
      <div>
        <p class="font-bold text-gray-900">$ {{ parseFloat(payment.divisa_amount).toFixed(2) }}</p>
